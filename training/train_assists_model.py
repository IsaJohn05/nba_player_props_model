...
DATA_PATH = "data/processed/train_assists.csv"
MODEL_PATH = "models/assists_xgb.json"

FEATURES = [
    # shared / role proxies (you already compute these in assists inference)
...
def main():
    ...
    y_train = train_df["delta_ast_per_min"]

    X_val = val_df[FEATURES]
    y_val = val_df["delta_ast_per_min"]

    # Baseline for delta is always 0 (i.e., predict ast_per_min_last10 with no adjustment)
    baseline_preds = [0.0] * len(y_val)
    baseline_mae = mean_absolute_error(y_val, baseline_preds)
    print(f"Baseline MAE (predict delta=0) on {VAL_SEASON}: {baseline_mae:.6f}")
    ...
    print(f"XGBoost MAE (delta_ast_per_min) on {VAL_SEASON}: {model_mae:.6f}")
    print(f"Improvement: {baseline_mae - model_mae:.6f} ast/min")
