...
PROPS_NORM = Path("data/odds_logs/rebounds_props_normalized.csv")
...
# Output
OUT_PATH = Path("data/processed/today_rebounds_prop_predictions.xlsx")
...
def build_latest_player_features(nba: pd.DataFrame) -> pd.DataFrame:
    """
    One row per player, rolling features (shift(1) to avoid leakage).
    Builds everything needed for:
      - minutes_xgb.json inputs
      - rebounds mean/sigma
    """
    nba = nba.copy()
    ...
    # Ensure numeric
    for c in ["MIN", "PTS", "REB", "FGA", "FTA", "FG3A", "TOV"]:
        if c in nba.columns:
            nba[c] = pd.to_numeric(nba[c], errors="coerce")

    if "REB" not in nba.columns:
        raise RuntimeError("NBA logs missing REB column.")

    rows = []
    for _, pdf in nba.groupby("PLAYER_ID"):
        pdf = pdf.sort_values("GAME_DATE")

        # Minutes model inputs (MUST match the trained feature set)
        ...
        # Rebounds model rate + volatility
        pdf["reb_per_min_last10"] = (
            pdf["REB"].shift(1).rolling(10).sum()
            / pdf["MIN"].shift(1).rolling(10).sum()
        )
        pdf["reb_std_last10"] = pdf["REB"].shift(1).rolling(10).std()
    ...
    keep = [
        ...
        "reb_per_min_last10", "reb_std_last10",
...
def archive_run_rebounds(out_xlsx_path: Path, props_norm_path: Path,
                         overs_tbl: pd.DataFrame, unders_tbl: pd.DataFrame,
                         tz: str = "America/Toronto") -> Path:
    run_date = pd.Timestamp.now(tz=tz).date().isoformat()
    arch_dir = Path("data/archives/rebounds") / run_date
    arch_dir.mkdir(parents=True, exist_ok=True)

    xlsx_dst = arch_dir / "rebounds_predictions.xlsx"
    if out_xlsx_path.exists():
        shutil.copy2(out_xlsx_path, xlsx_dst)

    props_dst = arch_dir / "rebounds_props_normalized.csv"
...
def main():
    ...
    # Filter to today's LOCAL date (Toronto) so we don't re-score yesterday
    ...
    # Rebounds mean + uncertainty
    pred_df["reb_mean"] = pred_df["pred_minutes"] * pred_df["reb_per_min_last10"]
    pred_df["sigma"] = pred_df["reb_std_last10"].astype(float).clip(lower=2.0).fillna(2.5)

    # Probabilities
    z = (pred_df["line"] - pred_df["reb_mean"]) / pred_df["sigma"]
    ...
    # Selection rules: top 11, max 5 unders, max 1 pick per player
    ranked = (
        pred_df.sort_values("ai_rating", ascending=False)
        .drop_duplicates(subset=["player_norm", "line"], keep="first")
        .drop_duplicates(subset=["player_norm"], keep="first")
        .copy()
    )

    unders = ranked[ranked["best_side"] == "UNDER"].head(5).copy()
    ...
    def build_display(df_part: pd.DataFrame) -> pd.DataFrame:
        return pd.DataFrame({
            ...
            "PROP": df_part.apply(lambda r: f"{r['best_side']} {r['line']} REBOUNDS", axis=1),
    ...
    # Write formatted Excel (same style)
    ...
    with pd.ExcelWriter(OUT_PATH, engine="openpyxl") as writer:
        ...
        title = f"@Jayssportsanalytics - NBA Player Rebounds Model - {pd.Timestamp.now().strftime('%m/%d/%Y')}"
        ...
        next_row = 2
        next_row = write_section(ws, next_row, "OVERS", overs_tbl, overs_bar_fill, overs_header_fill, overs_row_fill)
        next_row = write_section(ws, next_row, "UNDERS", unders_tbl, unders_bar_fill, unders_header_fill, unders_row_fill)
    ...
    # Archive
    archive_run_rebounds(OUT_PATH, PROPS_NORM, overs_tbl, unders_tbl)
