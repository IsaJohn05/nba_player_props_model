...
# Output
OUT_PATH = Path("data/processed/today_points_prop_predictions.xlsx")
...
def archive_run_points(
    ...
    """
    Archives today's run output + the exact input props snapshot + picks table.

    Creates:
      data/archives/points/YYYY-MM-DD/
        - points_predictions.xlsx
        - points_props_normalized.csv
        - picks.csv
    """
    run_date = pd.Timestamp.now(tz=tz).date().isoformat()
    arch_dir = Path("data/archives/points") / run_date
    arch_dir.mkdir(parents=True, exist_ok=True)

    # 1) Copy final Excel output
    xlsx_dst = arch_dir / "points_predictions.xlsx"
    if out_xlsx_path.exists():
        shutil.copy2(out_xlsx_path, xlsx_dst)

    # 2) Copy the props snapshot used
    props_dst = arch_dir / "points_props_normalized.csv"
...
def build_latest_team_defense(nba: pd.DataFrame) -> pd.DataFrame:
    """
    Opponent defense context from historical logs.
    """
    nba = nba.copy()
    nba["GAME_DATE"] = pd.to_datetime(nba["GAME_DATE"])

    team_col = "TEAM_ABBREVIATION" if "TEAM_ABBREVIATION" in nba.columns else "TEAM_NAME"
    if team_col not in nba.columns:
        raise RuntimeError("NBA logs missing TEAM_ABBREVIATION and TEAM_NAME; can't build opponent defense.")

    for c in ["PTS", "FGA", "FTA", "FG3A"]:
        nba[c] = pd.to_numeric(nba[c], errors="coerce")

    team_game = (
        nba.groupby(["GAME_ID", "TEAM_ID", team_col, "GAME_DATE"], as_index=False)
    ...
    m = team_game.merge(opp, on=["GAME_ID", "GAME_DATE"], how="inner")
    m = m[m["TEAM_ID"] != m["OPP_TEAM_ID"]].copy()
    m = m.sort_values(["TEAM_ID", "GAME_DATE"])

    m["opp_pts_allowed_last10"] = m["opp_pts_scored"].shift(1).rolling(10).mean()

    latest = m.groupby("TEAM_ID").tail(1).copy()
    latest["team_abbr"] = latest[team_col].astype(str).str.upper().str.strip()

    return latest[["team_abbr", "opp_pts_allowed_last10"]].drop_duplicates(subset=["team_abbr"], keep="last")
...
def main():
    ...
    # -----------------------
    # Filter props to today's LOCAL date (Toronto)
    # -----------------------
    ...
    if props.empty:
        raise RuntimeError(f"No games left after filtering to local date {today_local}.")
    ...
    # Opponent inference using CURRENT team + matchup
    ...
    df = df.merge(team_def, left_on="opp_team_abbr", right_on="team_abbr", how="left")
    ...
    # Points mean = minutes * rate
    pred_df["pts_mean"] = pred_df["pred_minutes"] * pred_df["pts_per_min_last10"]

    # Normal uncertainty
    pred_df["sigma"] = pred_df["pts_std_last10"].astype(float).clip(lower=4.0).fillna(6.0)

    # P(Over)
    z = (pred_df["line"] - pred_df["pts_mean"]) / pred_df["sigma"]
    ...
    # final combined set (optional debug)
    top = pd.concat([overs, unders], ignore_index=True).sort_values("ai_rating", ascending=False)
    ...
    def build_display(df_part: pd.DataFrame) -> pd.DataFrame:
        return pd.DataFrame({
            ...
            "PROP": df_part.apply(lambda r: f"{r['best_side']} {r['line']} POINTS", axis=1),
    ...
    with pd.ExcelWriter(OUT_PATH, engine="openpyxl") as writer:
        ...
        # Title row
        title = f"@Jayssportsanalytics - NBA Player Points Model - {pd.Timestamp.now().strftime('%m/%d/%Y')}"
        ...
        # COMPACT: Start OVERS directly under title (row 2)
        next_row = 2
        ...
        # COMPACT: UNDERS immediately after OVERS (no blank rows)
        next_row = write_section(
            ws=ws,
            start_row=next_row,
            title="UNDERS",
